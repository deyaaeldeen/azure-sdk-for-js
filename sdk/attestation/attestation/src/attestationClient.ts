...
export class AttestationClient {
  ...
  public async attestOpenEnclave(report : Uint8Array, options : AttestOpenEnclaveOptions={}) : Promise<AttestationResponse<AttestationResult>> {
    ...
    try {
      ...
      const token = new AttestationToken(attestationResult.token ?? "");
      const serializer = new Serializer({ AttestationResultMapper });

      const result = new AttestationResponse<AttestationResult>(token, serializer, AttestationResultMapper, "attestationResult", attestationResult);
      return result;
